...
/*
** ecm_boot.prg was written by Joakim Westerlund, Department of Economics, Lund University.
...
k       = 2;                      /*regressors, k = 6 is max */
t       = 28;                     /*time period*/ 
n       = 9;                      /*number of cross-section */  
nb      = 1000;                   /* no of bootstrap replications */
mod     = 3;                      /* 1 = nothing, 2 = const, 3 = const & trend */	        
p       = 1; //int(4*(t/100)^(2/9));    /* lags */
q       = 1; //int(4*(t/100)^(2/9));    /* leads */
r       = int(4*(t/100)^(2/9));    /*bandwidth*/

load y[t,n] 	= lnfd.txt;        /* t x n matrix */
load x[t,n*k] 	= west_x.txt;      /* t x n*k matrix */
...
"                                   asym   bootstrap";   
"                                   p-val   p-val";
"g_tau   = ";; g_tau   ;;(1-cdfnc(test[1]));; pval[1];
"g_alpha = ";; g_alpha ;;(1-cdfnc(test[2]));; pval[2];
"p_tau   = ";; p_tau   ;;(1-cdfnc(test[3]));; pval[3];
"p_alpha = ";; p_alpha ;;(1-cdfnc(test[4]));; pval[4];
...
if mod == 1; w = x;                       endif;
if mod == 2; w = x~ones(t,1);             endif;
if mod == 3; w = x~ones(t,1)~seqa(1,1,t); endif;                                  
...
ti = zeros(2,1);
do until i > n;
ti = real(ti + ecm(xb[.,1+(i-1)*k:i*k],yb[.,i],mod,p,q,r));        
i  = i + 1;
endo;
...
if mod == 2; x = x~ones(t-p-q-1,1);                   endif;
if mod == 3; x = x~ones(t-p-q-1,1)~seqa(1,1,t-p-q-1); endif;
...
if p; dyl = zeros(t-p-q-1,p); endif;			
do until j < -p;
...
yl = y[t1-1:t2-1];
dy = y[t1:t2]-yl;
if p ne 0; x = (yl~xl~dxl~dyl); endif;
if p eq 0; x = (yl~xl~dxl);     endif;
...
if mod == 2; xi = xi~ones(t-p-q-1,1);                   endif;
if mod == 3; xi = xi~ones(t-p-q-1,1)~seqa(1,1,t-p-q-1); endif;
